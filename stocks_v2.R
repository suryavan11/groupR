...
GetYLags <- function(features,k,n){
  
  for (i in seq_along(features)) {
    
    for ( j in k:n) {
      temp = lag.xts(features[[i]]$y,j)
      names(temp) = paste0('ylag',j)
      features[[i]] = merge(temp,features[[i]])
    }
...
# features = GetYLags(features,2,5)
...
#feature.list = c(feature.list, colnames(modeldf)[str_detect(colnames(modeldf), 'ylag')] )
...
xgb.grid <-  expand.grid(nrounds = c(100 ), ## 3000
                         max_depth = c(4), ## 6
                         eta = c( 0.01), ## 0.01
                         gamma = 0,
